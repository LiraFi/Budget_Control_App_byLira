    ...
    return (
        <Stack sx={{color: "text.primary"}} padding={2} spacing={3}>
            ...
            <Box sx={{position: "relative", display: "inline-flex"}}>
                <CircularProgress variant="determinate" value={savings / goal * 100} />
                <Box sx={{top: 11, left: 5, bottom: 0, right: 0, position: "absolute", alignItems: "center", justifyContent: "center"}}>
                <Typography variant="caption" component="div">{`${Math.round(savings / goal * 100)}%`}</Typography>
                </Box>
            </Box>
            <Box component="form">
                <TextField label="Set Goal" type="number" name="goal" onChange={(e) => setGoal(Number(e.target.value))}/>
            </Box>
        </Stack>
    )
}
